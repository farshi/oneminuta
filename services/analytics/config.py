...
from libs.config_loader import get_config as get_env_config, get_openai_api_key, get_telegram_api_id, get_telegram_api_hash
...
class AnalyticsConfig:
    ...
    def _load_config(self):
        ...
        self.LLM_MODEL: str = get_env_config('LLM_MODEL', default='gpt-4o-mini')
        self.LLM_TEMPERATURE: float = float(get_env_config('LLM_TEMPERATURE', default='0.1'))
        self.LLM_MAX_TOKENS: int = int(get_env_config('LLM_MAX_TOKENS', default='1000'))
        ...
        self.STORAGE_PATH: str = get_env_config('STORAGE_PATH', default='./storage')
        self.SESSION_PATH: str = get_env_config('SESSION_PATH', default='./sessions')
        
        # Alert Configuration
        self.WEBHOOK_URL: Optional[str] = get_env_config('WEBHOOK_URL')
        self.HOT_CLIENT_THRESHOLD: float = float(get_env_config('HOT_CLIENT_THRESHOLD', default='70.0'))
        self.BURNING_CLIENT_THRESHOLD: float = float(get_env_config('BURNING_CLIENT_THRESHOLD', default='85.0'))
        
        # Monitoring Configuration - from centralized config
        channels_env = get_env_config('TELEGRAM_CHANNELS', default='')
        if channels_env:
        ...
        languages_env = get_env_config('SUPPORTED_LANGUAGES', default='en,ru')
        self.SUPPORTED_LANGUAGES: List[str] = [lang.strip() for lang in languages_env.split(',')]
        self.DEFAULT_LANGUAGE: str = get_env_config('DEFAULT_LANGUAGE', default='en')
        
        # Analysis Configuration
        self.MAX_MESSAGES_PER_ANALYSIS: int = int(get_env_config('MAX_MESSAGES_PER_ANALYSIS', default='20'))
        self.MAX_STORED_MESSAGES: int = int(get_env_config('MAX_STORED_MESSAGES', default='50'))
        self.ANALYSIS_CACHE_TTL: int = int(get_env_config('ANALYSIS_CACHE_TTL', default='3600'))
    
    def _require_env(self, var_name: str) -> str:
        """Require an environment variable to be set (legacy method - use config loader instead)"""
        value = get_env_config(var_name, required=True)
        return value
